...
public class LevelMap extends CCTMXTiledMap {
	...
	public void birdJump()
	{
		if (_birdVy == 0)
		{
			...
			{
				_birdVy = com.sonetta.bow.fishing.G.jumpVy;
				if (!_isJumping)
				{
					_isJumping = true;
	...
	public void update(float dt)
	{
		...
		case tkLeft:
			if (_birdDir != -1) 
			{
				if( com.sonetta.bow.fishing.G.sound ) com.sonetta.bow.fishing.G.soundDirection.start();
				setBirdDir(-1);
			}
			break;
		case tkRight:
		...
		case tkNormalR:
			if (_birdDir == 1) 
			{
				if( com.sonetta.bow.fishing.G.sound ) com.sonetta.bow.fishing.G.soundSpeedDown.start();
				_birdVx = com.sonetta.bow.fishing.G.normalVx;
			}
			break;
		case tkNormalL:
			if (_birdDir == -1)
		...
		case tkFastR:
			if (_birdDir == 1)
			{
				if( com.sonetta.bow.fishing.G.sound ) com.sonetta.bow.fishing.G.soundSpeedUp.start();
				_birdVx = com.sonetta.bow.fishing.G.fastVx;
			}
			break;
		case tkFastL:
			if (_birdDir == -1)
	...
	{
		float x = com.sonetta.bow.fishing.G.width * (0.5f -_birdDir*0.25f) - _bird.getPosition().x;
		if (x > 0)
		{
			x = 0;
		}
		else if (x+contentSize_.width < com.sonetta.bow.fishing.G.width)
		{
			x = com.sonetta.bow.fishing.G.width - contentSize_.width;
		}

		float y = com.sonetta.bow.fishing.G.height*0.5f - _tileSize.height*0.5f - _bird.getPosition().y;
		setPosition(x, y);
	}

	public void gotCherry(CGPoint pos)
	{
		if( com.sonetta.bow.fishing.G.sound ) com.sonetta.bow.fishing.G.soundCollect.start();
		...
		cherry.runAction(CCSequence.actions(
			CCMoveTo.action(1, CGPoint.ccp(60, com.sonetta.bow.fishing.G.height-50)),
	...
	{
		if( com.sonetta.bow.fishing.G.sound ) com.sonetta.bow.fishing.G.soundCollide.start();
		_game.state = com.sonetta.bow.fishing.G.gsPause;
